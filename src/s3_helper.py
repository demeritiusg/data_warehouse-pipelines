...
class S3Module:
    ...
    def __init__(self):
        self._s3 = boto3\
                    .resource(service_name = '', region_name = '', aws_access_key_id=config.get('', ''), aws_secret_access_key=config.get('', ''))
        self._files = []
        self._landing_zone = config.get('','')
        self._working_zone = config.get('','')
        self._processed_zone = config.get('','')
    ...
    def s3_move_data(self, source_bucket = None, target_bucket= None):
        ...
        logging.debug('Inside s3_move_data : Source bucket set is : {source_bucket}\n Target bucket set is : {}'.format(target_bucket))
        ...
        for key in self.get_files(source_bucket):
            if key in config.get('','').split(','):
                logging.debug('Copying file {key} from {source_bucket} to {}'.format(target_bucket))
    ...
    def get_files(self, bucket_name):
        ...
        """
        logging.debug('Inspecting bucket : {} for files present'.format(bucket_name))
